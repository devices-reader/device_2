inherited frmSetup2: TfrmSetup2
  Left = 150
  Top = 250
  Caption = #1048#1085#1092#1086#1088#1084#1072#1094#1080#1103
  ClientHeight = 212
  ClientWidth = 341
  PixelsPerInch = 96
  TextHeight = 16
  inherited panClient: TPanel
    Width = 341
    Height = 171
    object lblInfo: TLabel
      Left = 56
      Top = 16
      Width = 174
      Height = 16
      Caption = #1059#1089#1090#1072#1085#1086#1074#1080#1090#1100' '#1074#1088#1077#1084#1103' '#1089#1095#1077#1090#1095#1080#1082#1072' ?'
    end
    object imgInfo: TImage
      Left = 16
      Top = 9
      Width = 32
      Height = 32
      AutoSize = True
      Picture.Data = {
        
        0000000000000000000100008003000080030000C0070000F01F0000}
    end
    object rgrInfo: TRadioGroup
      Left = 16
      Top = 96
      Width = 129
      Height = 65
      Items.Strings = (
        #1083#1077#1090#1085#1077#1077' '#1074#1088#1077#1084#1103
        #1079#1080#1084#1085#1077#1077' '#1074#1088#1077#1084#1103)
      TabOrder = 0
    end
    object medSetTime: TMaskEdit
      Left = 16
      Top = 48
      Width = 129
      Height = 24
      Enabled = False
      EditMask = '!99:99:99 99/99/99;1; '
      MaxLength = 17
      TabOrder = 1
      Text = '  :  :     .  .  '
    end
    object chbNow: TCheckBox
      Left = 16
      Top = 80
      Width = 249
      Height = 17
      Caption = #1080#1089#1087#1086#1083#1100#1079#1086#1074#1072#1090#1100' '#1074#1088#1077#1084#1103' '#1082#1086#1084#1087#1100#1102#1090#1077#1088#1072
      Checked = True
      Enabled = False
      State = cbChecked
      TabOrder = 2
      OnClick = chbNowClick
    end
  end
  inherited panBottom: TPanel
    Top = 171
    Width = 341
    inherited panRight: TPanel
      Left = 130
      inherited btbOK: TBitBtn
        Default = False
      end
      inherited btbNo: TBitBtn
        Default = True
      end
    end
  end
  object timNow: TTimer
    Interval = 300
    OnTimer = timNowTimer
    Left = 168
    Top = 48
  end
end
